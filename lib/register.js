...
extensions['.js'] = function (module, filename) {
  if (filename.startsWith(eslintRoot)) {
    ...
    if (replacer) {
      ...
      if (replaced === content)
        throw new Error(`[eslint-ts-patch] ${pathname} is not patched! It might because ESLint changes the internal code structure. Please report this to https://github.com/antfu/eslint-ts-patch`)
      debug(`patched ${pathname}`)
